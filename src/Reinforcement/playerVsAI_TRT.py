...
# --- จุดที่ 1: แก้ให้หา GameBoard เจอไม่ว่าจะรันจากไหน ---
...
# Logger สำหรับ TensorRT
TRT_LOGGER = trt.Logger(trt.Logger.WARNING)

# เปลี่ยนจาก TFLiteBrainWrapper เป็น TRTBrainWrapper
class TRTBrainWrapper:
    def __init__(self, engine_path):
        ...
        # 2. จองหน่วยความจำ (Allocation)
        self.inputs, self.outputs, self.bindings = [], [], []
        for binding in self.engine:
            size = trt.volume(self.engine.get_binding_shape(binding))
            dtype = trt.nptype(self.engine.get_binding_dtype(binding))
            # Host mem (CPU) และ Device mem (GPU)
    ...
    def predict(self, board):
    # 1. ดึง State ที่เป็น Relative Perspective (แก้ GameBoard ก่อนถึงจะเวิร์ก)
        state = board.getStateAsPlayer().astype(np.float32)
    
    # 2. เตรียม Input Data (NCHW: 1, 3, 6, 7)
        input_data = np.expand_dims(state, axis=0) 

    # 3. Transfer ข้อมูลไปยัง GPU
    # ใช้ .flatten() เพื่อให้ขนาดตรงกับ buffer ที่จองไว้
    ...
    # 5. ดึงข้อมูลกลับมายัง CPU
        for out in self.outputs:
            cuda.memcpy_dtoh_async(out['host'], out['device'], self.stream)
        self.stream.synchronize()

    # 6. คัดแยก Policy (7 ช่อง) ออกจาก Value (1 ช่อง)
    # TensorRT อาจสลับ Index กันได้ เราจึงต้องเช็คขนาด
        res0 = self.outputs[0]['host']
        res1 = self.outputs[1]['host'] if len(self.outputs) > 1 else res0
    
    # ถ้าขนาดเป็น 7 คือ Policy (ความน่าจะเป็นในการเดิน)
        if len(res0) == 7:
            policy = res0
        else:
            policy = res1
...
def play():
    ...
    # ระบุ Path ไปที่ไฟล์ .engine ที่เพิ่งสร้าง
    model_path = os.path.join(reinforcement_dir, "Models", "model_v5.engine")
